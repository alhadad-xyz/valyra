...
contract EscrowV1Test is Test {
    ...
    function test_DepositFunds() public {
        ...
        (uint256 platformFee, uint256 sellerPayout) = escrow.calculateFees(
            ESCROW_AMOUNT
        );
    ...
    function test_ConfirmReceipt() public {
        ...
        // Check balances
        assertEq(
            idrxToken.balanceOf(seller),
            sellerBalanceBefore + immediateRelease
        );
        assertEq(
            idrxToken.balanceOf(treasury),
        ...
        EscrowV1.TransitionHold memory hold = escrow.getTransitionHold(
            escrowId
        );
    ...
    function test_ClaimTransitionRetainer() public {
        ...
        assertEq(
            idrxToken.balanceOf(seller),
            sellerBalanceBefore + hold.retainedAmount
        );
    ...
    function test_RaiseDispute() public {
        ...
        assertEq(
            uint256(dispute.disputeType),
            uint256(EscrowV1.DisputeType.CREDENTIAL_MISMATCH)
        );
    ...
    function test_ResolveDisputeReleaseToSeller() public {
        ...
        assertEq(
            idrxToken.balanceOf(seller),
            sellerBalanceBefore + sellerPayout
        );
        assertEq(
            idrxToken.balanceOf(treasury),
            treasuryBalanceBefore + platformFee
