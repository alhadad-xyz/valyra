...
contract EscrowV1 is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuard, UUPSUpgradeable {
    ...
    event TransitionRetainerClaimed(
        uint256 indexed escrowId,
        uint256 amount
    );
    
    event TransitionIssueReported(
        uint256 indexed escrowId,
        string issue
    );
     ...
     */
    function initialize(address _treasuryAddress, address _idrxToken, address _marketplaceAddress) public initializer {
    ...
    function depositFunds(
        ...
        require(state == IMarketplace.ListingState.ACTIVE, "Listing not active");
        
        // Calculate fees
        (uint256 platformFee, uint256 sellerPayout) = calculateFees(amount);
        ...
        emit FundsDeposited(escrowId, msg.sender, amount, encryptionMethod);
    }
     ...
     */
    function uploadCredentialHash(uint256 escrowId, bytes32 credentialHash)
        external
        whenNotPaused
        ...
        nonReentrant
    {
        ...
        require(block.timestamp <= escrow.handoverDeadline, "Handover deadline passed");
     ...
     * @param escrowId ID of the escrow transaction
     */
    function confirmReceipt(uint256 escrowId)
        external
        whenNotPaused
        escrowExists(escrowId)
        onlyBuyer(escrowId)
        nonReentrant
    {
        ...
        require(block.timestamp <= escrow.verifyDeadline, "Verification deadline passed");
        ...
        emit FundsReleased(escrowId, escrow.seller, immediateRelease, escrow.platformFee);
     ...
     * @param escrowId ID of the escrow transaction
     */
    function claimTransitionRetainer(uint256 escrowId)
        external
        whenNotPaused
        escrowExists(escrowId)
        onlySeller(escrowId)
        nonReentrant
    {
        ...
        require(block.timestamp >= hold.releaseTime, "Transition period not ended");
     ...
     */
    function reportTransitionIssue(uint256 escrowId, string memory issue)
        external
        whenNotPaused
        escrowExists(escrowId)
        onlyBuyer(escrowId)
     ...
     * @param escrowId ID of the escrow transaction
     */
    function requestVerificationExtension(uint256 escrowId)
        external
        whenNotPaused
        escrowExists(escrowId)
        onlyBuyer(escrowId)
    {
        ...
        require(block.timestamp < escrow.verifyDeadline, "Deadline already passed");
    ...
    function raiseDispute(
        ...
        require(
            escrow.state == EscrowState.DELIVERED || escrow.state == EscrowState.TRANSITION,
     ...
     */
    function respondToDispute(uint256 escrowId, string memory responseIpfs)
        external
        whenNotPaused
        escrowExists(escrowId)
    {
    ...
    function resolveDispute(
        ...
        if (resolution == DisputeResolution.FULL_REFUND) {
            ...
            escrow.state = EscrowState.COMPLETED;
        } else if (resolution == DisputeResolution.PARTIAL_REFUND || resolution == DisputeResolution.SPLIT) {
            // Calculate split based on refundPercent
            uint256 refundAmount = (escrow.amount * refundPercent) / 10000;
            uint256 sellerAmount = escrow.amount - refundAmount - escrow.platformFee;
        ...
        nonReentrant
    {
        ...
        if (escrow.state == EscrowState.FUNDED) {
            ...
            require(block.timestamp > escrow.handoverDeadline, "Handover deadline not passed");
            ...
            require(block.timestamp > escrow.verifyDeadline, "Verification deadline not passed");
    ...
    function calculateFees(uint256 amount)
        public
        pure
        returns (uint256 platformFee, uint256 sellerPayout)
    {
     ...
     * @param escrowId ID of the escrow transaction
     */
    function getTransitionHold(uint256 escrowId)
        external
        view
        returns (TransitionHold memory)
    {
